...
public class RedisInitBean {
	...
	private static Logger logger = Logger.getLogger(RedisInitBean.class);

	public static void main(String[] args) {
		RedisInitBean bean = new RedisInitBean();
		List<String> list = new ArrayList<String>();
		list.add("aaa");
		list.add("aaa");
		//bean.addList("test:aaa", list);
		//bean.addListr("test:aaa", "bb");
		bean.addSet("test:set", "aset");
		String json = bean.jedis.hget("ykat:tk:usertk:5010ad0fcf5850c34ec3bd11714f13e1", "user");
		System.out.println(json);
	}
	
	public RedisInitBean() {
		super();
		//Host.add("10.254.250.160:6379");
		Host.add("192.168.10.23:6379");
		this.maxWaitMillis = 1000;
		MaxIdle = 200;
		this.testOnBorrow = true;
		if (Host.size() != 0) {
			for (int i = 0; i < Host.size(); i++) {
				String h[] = ((String) Host.get(i)).split(":");
				JedisShardInfo shardinfo = 	new JedisShardInfo(h[0].trim(), Integer.parseInt(h[1].trim()));
				shardinfo.setPassword("123456");
				shards.add(shardinfo);
			}
		} else {
			log.error("请检查Redis配置，host项为必填项！格式[IP:PORT]");
		}
...
//	    }
    ...
    public boolean delSetValue(String key, String value) {
    	  ShardedJedis shardedJedis = null;
          try {
              shardedJedis = shardedJedisPool.getResource();
              shardedJedis.srem(key, value);
              ...
              logger.error("delSetValue error.", ex);
              returnBrokenResource(shardedJedis);
          } finally {
              returnResource(shardedJedis);
          }
    ...
    public boolean spopSetValue(String key) {
  	  ShardedJedis shardedJedis = null;
        try {
            shardedJedis = shardedJedisPool.getResource();
            ...
            logger.error("spopSetValue error.", ex);
            returnBrokenResource(shardedJedis);
        } finally {
            returnResource(shardedJedis);
        }
